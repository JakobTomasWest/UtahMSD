    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin( string name, int cardnum )
        {
            ...
            var query = from p in db.Patrons
                        ...
                        select p;
            var matchingProfile = query.SingleOrDefault();
        ...
        {
            // LINQ has to get from before selecting
            var query =
                from t in db.Titles
                join i in db.Inventory on t.Isbn equals i.Isbn into inv
                from i in inv.DefaultIfEmpty()
                join c in db.CheckedOut on i.Serial equals c.Serial into co
                from c in co.DefaultIfEmpty()
                join p in db.Patrons on c.CardNum equals p.CardNum into pat
                from p in pat.DefaultIfEmpty()
                select new
                {
                    isbn = t.Isbn,
                    title = t.Title,
        ...
        public ActionResult CheckOutBook( int serial )
        {
            ...
            var alreadyCheckedOut=
                (from co in db.CheckedOut
                 where (uint)serial == co.Serial
                 select co).SingleOrDefault();
            

            if (alreadyCheckedOut == null)
            {   
                var bookToCheckout = new CheckedOut
            {
                CardNum = (uint)card,
                Serial = (uint)serial
            };
                db.CheckedOut.Add(bookToCheckout);
                db.SaveChanges();
                return Json(new { success = true });
            }
            else
        ...
        {
            // You may have to cast serial to a (uint)
            // Find the checked out book and if accessor Patron...Card is the same as the cardnum
            // And they checkedout that serial grab the book
            var coBook =
                (from c in db.CheckedOut
                 where c.Serial == (uint)serial && c.CardNum == card
                 select c).SingleOrDefault();
            // If we were able to grab the single element... COBook and its not null,
            // lets return the book, removing it from the Checkedout DB table
            if (coBook != null)
            {
                db.CheckedOut.Remove(coBook);
                db.SaveChanges();
                return Json(new { success = true });
            }
            else
