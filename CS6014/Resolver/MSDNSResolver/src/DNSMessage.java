...
//        }
    ...
    public String[] readDomainName(InputStream inputStream) throws IOException {
        ...
        while(length != 0){
            ...
            if((length & 0xC0) == 0xC0) { // if length that was previously mentioned is now 0x CO to FF, we have a pointer - 1100 0000 to 1111 1111 because
                ...
                String[] pointerParts = readDomainNameFromOffset(offset);
                // add the array of pointer parts as a list to our arraylist
                domainNamePieces.addAll(Arrays.asList(pointerParts));
                break;
            }
            else {
    ...
    public String[] readDomainNameFromOffset(int offset) throws IOException {
        ...
        try (ByteArrayInputStream stream = new ByteArrayInputStream(messageBytes, offset, messageBytes.length - offset)) {
            return readDomainName(stream);
        }
    }
